...
const initRevenueChart = () => {
    if (document.querySelector("#newrevenueChart")) {
        document.querySelector("#newrevenueChart").innerHTML = ''; // clear previous rendering if any
        var options = {
            chart: {
                height: 365,
                type: 'line',
        ...
        var revenueChart = new window.ApexCharts(document.querySelector("#newrevenueChart"), options);
        revenueChart.render();
    }
};

const initCampaignsChart = () => {
    if (document.querySelector("#newcampaignsChart")) {
        document.querySelector("#newcampaignsChart").innerHTML = ''; // clear previous rendering if any
        var options = {
            series: [44, 55, 67],
            chart: {
                height: 350,
        ...
        var campaignsChart = new window.ApexCharts(document.querySelector("#newcampaignsChart"), options);
        campaignsChart.render();
    }
};

onMounted(() => {
    // Wait for the next tick to ensure DOM is ready, and wait a bit for ApexCharts script to load via AdminLayout if navigating directly
    nextTick(() => {
        setTimeout(() => {
            if (window.ApexCharts) {
                ...
                console.error("ApexCharts is not available globally.");
            }
        }, 300);
    });
});
